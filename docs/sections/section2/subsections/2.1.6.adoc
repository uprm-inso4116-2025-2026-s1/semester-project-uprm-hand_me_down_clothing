...
Notes:: Returns `None` if Listing missing/closed.
...
Notes:: Returns `None` if Piece missing/closed. Does not change visibility.
...
Postconditions:: Listing marked as discarded/closed.
...
6. Buyer leaves a review using `submitReview(Seller, Buyer, Review)` → produces `ReviewSubmitted`.
