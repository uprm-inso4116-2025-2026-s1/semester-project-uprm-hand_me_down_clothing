...
- *Action* — Deactivating or closing a listing once the item is no longer available.
...
| Reservation | Value Object | *Conceptually associates a Buyer with a Piece*; storage may use an internal identifier (e.g., buyerId) but the domain concept is *Buyer*.
...
[NOTE]
====
*Domain Purity:* Read-model/presentation types such as `Page<PieceSummary>` belong to query/view layers.
They must not be part of the *Piece* aggregate itself.
====
...
- `addReview(reviewer, rating, comment)` — attach feedback post-close.
...
- Read-model queries *(projections)* for UI lists, e.g.:
  ...
  - `findBySeller(seller) → Page<PieceSummary>`
