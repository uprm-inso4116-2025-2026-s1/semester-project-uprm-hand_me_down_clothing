...
==== [.added]#Domain Aggregate Definition# 

[.added]#The unified model of entities, actions, and events described above naturally forms bounded clusters of related concepts.# 
[.added]#In the context of the clothing exchange domain, the **Piece** (representing a listing) acts as the **aggregate root**, governing all changes to its internal state and related sub-entities such as condition ratings, reviews, and reservations.#

===== [.added]#Objective#
[.added]#Document the domain aggregate aligned with a discovery-only platform. For this milestone, **Piece** is treated as the **aggregate root** that also fulfills the “listing” role.#
[.added]#All mutations to elements inside the boundary occur **only** through the root.#

===== [.added]#Aggregate Definition#
...
[NOTE]
====
[.added]#**Boundary Rule:** Client code obtains a **Piece** (aggregate root), invokes root-level operations (e.g., `reserveFor`, `close`, `addReview`), and persists the **Piece** via a repository that works at the **aggregate** level. No direct writes to inner members.#
====
...
===== [.added]#Root Operations (Conceptual Surface)#
- [.added]#`publish()` — (if drafts are modeled) make the Piece publicly visible.#
- [.added]#`editDetails(updates)` — update title/description/attributes (forbidden when `Closed`).#
- [.added]#`rateCondition(newRating)` — set/adjust condition in `[1..10]`.#
- [.added]#`reserveFor(buyerId)` — `Active → Reserved` (hold while parties coordinate offline).#
- [.added]#`releaseReservation()` — `Reserved → Active`.#
- [.added]#`close(reason)` — `Active/Reserved → Closed` after offline handoff or withdrawal.#
- [.added]#`addReview(reviewerId, rating, comment)` — attach feedback post-close.#

===== [.added]#Factory (Conceptual)#
[.added]#The **Piece Factory** constructs valid roots and applies initial invariants:#
- [.added]#Ensures **≥ 1 image**.#
- [.added]#Ensures **condition in `[1..10]`**.#
- [.added]#Initializes **status = `Active`**, empty reviews, and no reservation.#

===== [.added]#Repository (Conceptual)#
[.added]#A repository that operates at the **aggregate level** (reads may provide projections for lists):#

- [.added]#`findById(pieceId) → Piece?`#
- [.added]#`save(piece) → void` (persists the whole aggregate and its members)#

- [.added]#Read-model queries (projections for UI lists), e.g.:#
  - [.added]#`findActiveByFilters(filters) → Page<PieceSummary>`#
  - [.added]#`findBySeller(sellerId) → Page<PieceSummary>`#