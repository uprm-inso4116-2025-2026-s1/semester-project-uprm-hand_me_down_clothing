...
== Executive Summary:
Monolithic domain models—where the entire business domain (users, listings, inventory, shipping, sizing, grades/conditions, donations, payments, messaging, etc.) is modeled together—can be simpler to start but produce serious long-term costs.  
...
. *Size Representation Conflict*: Listings store `size: "M"`, inventory uses numeric values, and search expects standardized sizes.
...
* Identify subdomains (Catalog, Listings, Inventory, Donations, Shipping, Payments, Messaging, Profiles).
...
image::MonolithicModel.png[alt="Monolithic architecture diagram", width=300, align=center]

=== B. Modular Monolith (Bounded Contexts):
image::ModularMonolith.png[alt="Modular monolith architecture diagram", width=300, align=center]
