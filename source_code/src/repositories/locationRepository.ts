...
export class LocationRepository {
    ...
    async fetchLocationByID(id : number) : Promise<Location>{
        const { data, error } = await this.supabase.from("stores").select("*").eq("id",id.toString());
        if(error){
        ...
        if(data.length>1){
            throw new Error(`Could not fetch location by ID. Duplicates Found found for ID: ${id}`);
        }
        const domainFormattedLocation: Location = LocationFactory.toDomainFormat(data[0]);
    ...
    async createLocation(loc : Location) : Promise<void>{
        const flattenedLocation : PersistenceLocation= LocationFactory.toPersistenceFormat(loc);
        const { error } = await this.supabase.from("stores").insert([flattenedLocation]);
        if(error){
            throw new Error(`Failed to create location: ${error.message}`);
        }
        console.log(`Location, name: ${flattenedLocation.name},  successfully added to Supabase.`)
    }

    async removeLocation(loc : Location) : Promise<void>{
        const flattenedLocation : PersistenceLocation= LocationFactory.toPersistenceFormat(loc);
        const { error } = await this.supabase.from("stores").delete().eq("id",flattenedLocation.id);
        if(error){
            throw new Error(`Failed to delete location. Location ID: ${flattenedLocation.id}. \nError message: ${error.message}`)
        }
    ...
    async updateLocation(loc: Location) : Promise<void>{
        const flattenedLocation : PersistenceLocation= LocationFactory.toPersistenceFormat(loc);
        const { data, error } = await this.supabase.from("stores").update(flattenedLocation).eq("id",flattenedLocation.id).select();
        if(error){
            throw new Error(`Could not update location. ID: ${flattenedLocation.id}. ${error.message}`)
        }
        if(data.length===0){
            throw new Error(`Could not update location. Location not found in Supabase.`)
        }
